# RetinaNet with residual objectness
import logging
import math
...
class ResObjRetinaNet(DenseDetector):
    """
    Implement ResObjRetinaNet in :paper:`Residual Objectness for Object Detection`.
    """
    ...
    def from_config(cls, cfg):
        ...
        head = ResObjRetinaNetHead(cfg, feature_shapes)
    ...
    def forward_training(self, images, features, predictions, gt_instances):
        ...
        pred_objs, pred_logits, pred_anchor_deltas = self._transpose_dense_predictions(
            predictions, [1, self.num_classes, 4]
        )
        anchors = self.anchor_generator(features)
        gt_labels, gt_boxes = self.label_anchors(anchors, gt_instances)
        return self.losses(anchors, pred_objs, pred_logits, gt_labels, pred_anchor_deltas, gt_boxes)

    def losses(self, anchors, pred_objs, pred_logits, gt_labels, pred_anchor_deltas, gt_boxes):
        """
        Args:
        ...
        obj_normalizer = self._ema_update("loss_normalizer", max(num_pos_anchors, 1), 100) * 20
        cls_normalizer = self._ema_update("loss_normalizer", max(num_pos_anchors, 1), 100) * 10
        reg_normalizer = self._ema_update("loss_normalizer", max(num_pos_anchors, 1), 100)

        # objectness, classification, and regression loss
        loss_obj = F.binary_cross_entropy_with_logits(
            cat(pred_objs, dim=1)[valid_mask].squeeze_(),
            pos_mask[valid_mask].float(),
            reduction="sum",
        )

        gt_labels_target = F.one_hot(gt_labels[pos_mask], num_classes=self.num_classes + 1)[
            :, :-1
        ]  # no loss for the last (background) class
        loss_cls = F.binary_cross_entropy_with_logits(
            cat(pred_logits, dim=1)[pos_mask],
            gt_labels_target.float(),
            reduction="sum",
        )
        ...
        return {
            "loss_obj": loss_obj / obj_normalizer,
            "loss_cls": loss_cls / cls_normalizer,
            "loss_box_reg": loss_box_reg / reg_normalizer,
        }
    ...
    def forward_inference(
        ...
        for img_idx, image_size in enumerate(images.image_sizes):
            scores_per_image = [o[img_idx].sigmoid_() * c[img_idx].sigmoid_() \
                for o, c in zip(pred_objs, pred_logits)]
...
class ResObjRetinaNetHead(nn.Module):
    """
    ...
    def __init__(
        ...
        *,
        ...
        assert norm == ""
        self.obj_subnet =  nn.Sequential(
            nn.Conv2d(out_channels, out_channels, kernel_size=3, stride=1, padding=1),
            nn.ReLU()
        )
        self.res_subnet =  nn.Sequential(
            nn.Conv2d(out_channels, out_channels, kernel_size=3, stride=1, padding=1),
            nn.ReLU()
        )
        ...
        self.obj_score = nn.Conv2d(
            conv_dims[-1], num_anchors, kernel_size=3, stride=1, padding=1
        )
        self.res_score = nn.Conv2d(
            conv_dims[-1], num_anchors, kernel_size=3, stride=1, padding=1
        )

        # Initialization
        for modules in [self.cls_subnet, self.bbox_subnet, self.cls_score, self.bbox_pred, 
            self.obj_score, self.res_score]:
        ...
        torch.nn.init.constant_(self.obj_score.bias, bias_value)
        torch.nn.init.constant_(self.res_score.bias, bias_value)
    ...
    def forward(self, features: List[Tensor]):
        ...
        for feature in features:
            ...
            obj_feature = self.obj_subnet(cls_feature)
            res_feature = self.res_subnet(obj_feature)
            objs.append(self.obj_score(obj_feature) + self.res_score(res_feature))
            logits.append(self.cls_score(cls_feature))
            bbox_reg.append(self.bbox_pred(self.bbox_subnet(feature)))

        return objs, logits, bbox_reg